...
def dist_log(text):
    if dist.is_available() and dist.is_initialized():
        rank = dist.get_rank()
        filename = RUNROOT / f"manual_{rank}.txt"
    else:
        filename = RUNROOT / "manual.txt"
...
class MetricsLoggerDebug(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer, model):
        if not dist.is_available() or not dist.is_initialized():
            warning_print(
                "Dist has not been initalized yet. Skipping on_validation_epoch."
            )
            return

        world_sz = dist.get_world_size()
        my_rank = dist.get_rank()
        with t.cuda.device(my_rank):
            cuda = t.device("cuda")
            y = np.array(reduce(lambda acc, ary: acc + ary, self.val_y, []))
            y_hat = np.array(reduce(lambda acc, ary: acc + ary, self.val_y_hat, []))
            my_y = t.tensor([t.from_numpy(y), t.from_numpy(y_hat)]).to(cuda)
            dprint(f"my_y.shape={my_y.shape}")
            mylen = y.shape[0]
            lens = [t.empty(1, dtype=t.int8).to(cuda) for _ in range(world_sz)]
            dist.all_gather(lens, t.tensor([mylen], dtype=t.int8).to(cuda))
            lens = list(map(lambda l: l.item(), lens))
            dprint(f"lens={lens}")
            ys = [None] * world_sz
            for rank in range(world_sz):
                if rank == my_rank:
                    # broadcast my tensors to everybody
                    dist.broadcast(my_y, src=my_rank)
                    ys[rank] = my_y
                else:
                    # get tensor from this rank
                    their_y = t.empty(2, lens[rank].item()).to(cuda)
                    dist.broadcast(their_y, src=rank)
                    ys[rank] = their_y
            for y_ in ys:
                dprint(f"y_.shape={y_.shape}")

        acc = np.mean((y_hat == y).astype(int))

        acc_dbg = self.val_acc_dbg.compute()

        dist_log(f"Epoch {self.val_epoch}: Val accuracy = {acc}, {acc_dbg.item()}")
    ...
    def on_train_epoch_end(self, trainer, model):
        ...
        dist_log(f"Epoch {self.train_epoch}: Train accuracy = {acc}, {acc_dbg.item()}")
...
def train():
    ...
    trainer = pl.Trainer(
        ...
        max_epochs=N_EPOCHS,
        callbacks=[MetricsLoggerDebug(RUNROOT), TQDMProgressBar(refresh_rate=0)],
