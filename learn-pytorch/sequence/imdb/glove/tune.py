...
def trial(hparams):
    ...
    glove_vecs = tt.vocab.GloVe(
        name="6B", dim=hparams["embedding_dim"], cache=DATAROOT / "glove"
    )
    imdb_vecs = glove_vecs.get_vecs_by_tokens(vocab.get_itos())
    simple_glove = SimpleGlove(imdb_vecs, hparams["max_seq_len"])
    model = LitImdb(
        simple_glove,
    ...
    with warnings.catch_warnings():
        ...
        trainer = pl.Trainer(
            ...
            callbacks=[
                ModelCheckpoint(filename=checkpoint_filename, monitor="val_acc")
            ],
            logger=logger,
...
@click.command()
@click.option(
    "--total-trials", default=3, show_default=True, help="Number of trials run by Ax."
)
@click.option(
    "--max-epochs",
    default="3,10",
...
@click.option("--batch-size", default="16,32,64", show_default=True, help="Batch size.")
@click.option(
    "--embedding-dim",
